...
**`scripts/agents/researcher/index.md:11`에 `targeted` mode가 이미 정의되어 있다.** `scripts/agents/researcher.md:38-91`에 분석 절차와 research.md 섹션 분기까지 완성되어 있다. `scripts/agents/researcher/hypothesis.md:3`은 *"좁은 범위 요청에서는 가설 절차를 적용하지 않고 관련 파일과 직접 의존성만 분석한다"*를 명시한다.
