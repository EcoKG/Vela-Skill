...
function generatePdf(data, outputPath) {
  return new Promise((resolve) => {
    ...
    doc
      .fontSize(12)
      .fillColor("#666666")
      .text((data.generatedAt || new Date().toISOString()).split("T")[0], {
        align: "center",
      });
    ...
    if (data.selectedItems) {
      doc
        .fontSize(10)
        .fillColor("#888888")
        .text(`Analysis scope: ${data.selectedItems.join(", ")}`, {
          align: "center",
        });
      doc.fillColor("#000000");
    }
    ...
    if (hasDeps) {
      ...
      doc.text(
        `  Critical: ${bySev.critical || 0}    High: ${bySev.high || 0}    Moderate: ${bySev.moderate || 0}    Low: ${bySev.low || 0}    Info: ${bySev.info || 0}`,
      );
      ...
      if (findings.length === 0) {
        ...
        for (const sev of severityOrder) {
          ...
          doc
            .fontSize(13)
            .fillColor(sevColors[sev] || "#000000")
            .text(`${sev.toUpperCase()} (${grouped[sev].length})`, {
              underline: false,
            });
          doc.fillColor("#000000");
          doc.moveDown(0.3);
          ...
          for (const f of grouped[sev]) {
            ...
            doc.text(
              `  Direct: ${f.isDirect ? "Yes" : "No"}    Fix Available: ${f.fixAvailable ? "Yes" : "No"}`,
            );
      ...
      doc
        .fontSize(16)
        .fillColor("#000000")
        .text("Outdated Packages", { underline: true });
      doc.moveDown(0.5);
    ...
    if (data.codeAnalysis) {
      ...
      doc
        .fontSize(16)
        .fillColor("#000000")
        .text("Code Analysis", { underline: true });
      doc.moveDown(0.5);
      ...
      if (
        ...
        doc
          ...
          .fillColor("#CC0000")
          .text(
            `Code analysis failed: ${data.codeAnalysis.error || "Unknown error"}`,
          );
        doc.fillColor("#000000");
      } else {
        ...
        for (const p of perspectives) {
          ...
          doc
            ...
            .fillColor("#666666")
            .text(
              `  Findings: ${(p.findings || []).length}    Cost: $${(p.cost || 0).toFixed(3)}    Duration: ${((p.durationMs || 0) / 1000).toFixed(1)}s`,
            );
          doc.fillColor("#000000");
          doc.moveDown(0.3);
          ...
          for (const f of pFindings) {
            ...
            doc
              .fillColor(sevColors[sev] || "#000000")
              .text(
                `  [${sev.toUpperCase()}] ${f.name || f.description || "Finding"}`,
                { continued: false },
              );
            doc.fillColor("#000000");
            if (f.file) {
              doc
                .fontSize(10)
                .text(`    File: ${f.file}${f.line ? ":" + f.line : ""}`);
            }
            if (f.description) {
            ...
            if (f.suggestion) {
              doc
                .fontSize(10)
                .fillColor("#336633")
                .text(`    Fix: ${f.suggestion}`, { width: 450 });
              doc.fillColor("#000000");
            }
            doc.moveDown(0.4);
          }
        ...
        doc.text(
          `Perspectives analyzed: ${okPerspectives.length}/${perspectives.length}`,
        );
        if (data.codeAnalysis.totalCost != null) {
          doc.text(
            `Total analysis cost: $${data.codeAnalysis.totalCost.toFixed(3)}`,
          );
        }
      }
    }
