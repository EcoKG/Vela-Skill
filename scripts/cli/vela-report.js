...
function extractPipeline(dirPath, date, slug) {
  ...
  try {
    ...
    const artifacts = allFiles.filter(f => f.endsWith('.md') || f.endsWith('.json') || f.endsWith('.patch'));
    ...
    return {
      date, slug, status: state.status, type: state.pipeline_type,
      request: state.request, step: state.current_step,
      completed: completedSteps.length, total: steps.length,
      created: state.created_at, updated: state.updated_at,
      artifacts, git: state.git || null,
      // Enhanced fields
      steps, completedSteps, cost, approvals
    };
  } catch (e) { return null; }
}
...
if (htmlOutput) {
  const statusColor = s => s === 'completed' ? '#22c55e' : s === 'active' ? '#3b82f6' : s === 'pending' ? '#64748b' : '#ef4444';
  ...
  const failedCount = pipelines.filter(p => p.status !== 'completed' && p.status !== 'active' && p.status !== 'pending').length;

  // Build step status lookup per pipeline
  function getStepStatus(pipeline, stepName) {
    if (pipeline.completedSteps.some(cs => (typeof cs === 'string' ? cs : cs.name) === stepName)) return 'completed';
    if (pipeline.step === stepName) return 'active';
    return 'pending';
  }
  function getStepDuration(pipeline, stepName) {
    const cs = pipeline.completedSteps.find(c => typeof c === 'object' && c.name === stepName);
    return cs && cs.duration ? cs.duration : '-';
  }

  // Generate paired rows: summary + expandable detail
  const rows = pipelines.map((p, idx) => {
    ...
    const artifactList = p.artifacts.length > 0
      ? p.artifacts.map(a => `<span class="artifact-tag">${a}</span>`).join(' ')
      : '<span style="color:#64748b">No artifacts</span>';

    // Cost display
    const costDisplay = p.cost != null ? `$${Number(p.cost).toFixed(4)}` : '-';

    // Approval scores
    const approvalDisplay = p.approvals && p.approvals.length > 0
      ? p.approvals.map(a => `${a.score || a.result || 'reviewed'}`).join(', ')
      : '';
