...
Explore 모드에서 사용자 질문에 답변할 때 반드시 따른다:

1. **팩트 검증 필수** — 내부 추론만으로 답변 금지. 코드 질문은 Read/Grep/Glob으로 실제 코드를 확인한다. 외부 정보(라이브러리, API 스펙, 공식 문서)가 필요하면 WebSearch/WebFetch를 사용한다. 검증 없이 추측으로 답변하는 것은 금지한다.
2. **웹서치 허용** — Explore 모드에서 WebSearch와 WebFetch를 사용할 수 있다. 외부 문서, 버전 정보, 공식 스펙 확인에 적극적으로 활용한다.
3. **이중 검토** — 답변을 작성한 후, 사용자에게 전달하기 전에 한 번 더 검토한다: 사실 오류가 없는지, 누락된 맥락이 없는지, 잘못된 가정이 없는지 확인한다.
